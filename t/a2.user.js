...
// @updateURL    https://upyun.learm.top/t/a2.user.js
...
if (url == '/login/gologin' && top == self) {
    ...
    $.tmDialog.alert({content: '2.X版本已取消支持旧版界面', title: '系统提示'});
...
function totalTime() {
    ...
    $.getScript('//cdn.jsdelivr.net/gh/sentsin/layer/dist/layer.js', function() {
        _self.layer.open({content: '已达到挂机限制时间', title: '系统提示'});
    });
}
...
function beforeFind() {
    _self.XMLHttpRequest = function() {
        ...
        ajax.onload = function(e) {
            if (this.status != 200 || !this.responseURL.match(/doHomework/)) return;
...
function fillAnswer($TiMu, obj, type) {
    ...
    obj.code > 0 && $div.each(function() {
        ...
        if (tip.match(/^(正确|是|对|√|T|ri)$/)) {
            ...
            Boolean($.inArray(tip, data) + 1 || str.match(tip)) == $input.checked || setting.queue.push($input);
        } else {
