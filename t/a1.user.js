...
// @namespace    learm.cn
// @version      0.3.10.1
// @description  自动化定义参数，视频解锁，分析题库 服务器blog.learm.top
// @author       learm
// @match        *://*.chaoxing.com/*
// @match        *://*.edu.cn/*
// @run-at       document-end
...
// @updateURL    https://upyun.learm.top/t/a1.user.js
// @supportURL   https://blog.learm.cn
// @license      MIT
// ==/UserScript==
//
//==========================================
...
if (url == '/mycourse/studentstudy') {
    ...
    _self.layui.use('layer', function() {
        this.layer.open({content: '尽量避免拖动进度条、倍速播放、秒过！', title: 'LeaRM', btn: '我明白', offset: 't', closeBtn: 0});
    });
        ...
        onload: function(xhr) {
            if (!setting.loop) {
                ...
                if (obj.code) {
                    ...
                    setting.div.children('div:eq(0)').html(obj.data || '服务器繁忙，小黑屋25秒');
                }
                setting.div.children('span').html(obj.msg || '');
            } else if (xhr.status == 403) {
                setting.div.children('div:eq(0)').data('html', '请求频繁，小黑屋30秒').siblings('button:eq(0)').click();
            } else {
                setting.div.children('div:eq(0)').text('连接异常，正在重试...');
            }
        },
        ontimeout: function() {
